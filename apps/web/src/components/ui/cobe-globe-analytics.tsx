...
/** Deskzy accent #1f6b57 as RGB 0–1 */
const ACCENT: [number, number, number] = [0.12, 0.42, 0.34];
const GLOW: [number, number, number] = [0.95, 0.94, 0.91];
    ...
    const interval = setInterval(() => {
      setData((prev) =>
        prev.map((m) => ({
          ...
          visitors: Math.max(
            1,
            m.visitors + Math.floor(Math.random() * 11) - 3,
          ),
          trend: Math.max(
            -20,
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      globe = createGlobe(canvas, {
        ...
        arcs: [
          {
            from: initialMarkers[0]?.location ?? [40.71, -74.01],
            to: initialMarkers[1]?.location ?? [51.51, -0.13],
          },
          {
            from: initialMarkers[1]?.location ?? [51.51, -0.13],
            to: initialMarkers[2]?.location ?? [35.68, 139.65],
          },
        ].filter((a) => a.from && a.to),
        arcColor: [0.2, 0.55, 0.45],
        arcWidth: 0.5,
        arcHeight: 0.25,
        opacity: 0.7,
      });
